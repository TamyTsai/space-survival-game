...
class Player(pygame.sprite.Sprite): # 表示創建Player物件類別(玩家操縱的飛船) self物件 #Player物件 繼承 內建的Sripte這個類別(pygame.sprite.Sprite為Sprite類別的位置)
    ...
    def __init__(self): #_init_表示初始函式 self表示物件本身
        ...
        self.rect = self.image.get_rect() # 將本類別image屬性，用get_rect() (定位用函式)框起來(框起來後可以設定一些屬性(中間、上下左右、右上右下左上(xy座標)左下...)，設定image要在框框中的甚麼位置)後，指定給 屬性rect
        # 將image(玩家飛船)碰撞邊界畫成圓
...
while running: #條件為真時，執行以下程式碼 #若遊戲進行中:
    ...
    draw_text(screen, str(score), 18, WIDTH/2, 10) # str(score):把分數轉為字串
