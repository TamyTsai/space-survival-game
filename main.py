...
while running: #條件為真時，執行以下程式碼 #若遊戲進行中:
    ...
    hits = pygame.sprite.spritecollide(player, rocks, False) #spritecollide(裏頭物件被碰撞的群組1, 裏頭物件被碰撞的群組2, 被碰撞到的群組2內的物件是否要被刪除) #撞到就要關閉遊戲了 所以要不要刪除物件意義不大 #python中比較特別，變數名稱相同時，後指定的值，不會影響前面指定給同名變數的值
